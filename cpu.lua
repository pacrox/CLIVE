...
function M.i4004_i1(rom, ram, shiftr1, shiftr2)-- >{
	...
	pin[15] = function(addr_space)	-- WIRE ROM	-->{
		rom = addr_space or rom
		return rom
	end-- >}

	local ram = ram
	pin[14] = function(addr_space)  -- WIRE RAM	-->{
		ram = addr_space or ram
		return ram
	end-- >}
	...
	local STEP = function()	-- STEP		-->{
		...
		print(string.format("PC=%03X FETCH=0x%02X", st[1], opr))  -- DEBUG
		st[1] = st[1] + 1		-- increase PC
	...
	opset[0x03+1] = function() 		-- JCN [ZC] 0011
		step_cycl = step_cycl + 1
		if reg[1] == 0 or flag[1] == 1 then -- A is zero or flag CARRY
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x05+1] = function() 		-- JCN [TZ] 0101
		step_cycl = step_cycl + 1
		if reg[1] == 0 or flag[2] == 1 then -- A is zero or flag ext-pin 
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x06+1] = function() 		-- JCN [TC] 0110
		step_cycl = step_cycl + 1
		if flag[1] == 1 or flag[2] == 1 then -- flag carry or flag ext-pin 
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x07+1] = function() 		-- JCN [TCZ] 0111
		step_cycl = step_cycl + 1
		if reg[1] == 0 or flag[1] == 1 or flag[2] == 1 then 
	...
	opset[0x0A+1] = function() 		-- JCN [IC] 1010
		step_cycl = step_cycl + 1
		if not(flag[1] == 1) then -- flag CARRY
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x0B+1] = function() 		-- JCN [IZC] 1011
		step_cycl = step_cycl + 1
		if not(reg[1] == 0 or flag[1] == 1) then -- A is zero or flag CARRY
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x0C+1] = function() 		-- JCN [IT] 1100
		step_cycl = step_cycl + 1
		if not(flag[2] == 1) then -- flag ext-pin 
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x0D+1] = function() 		-- JCN [ITZ] 1101
		step_cycl = step_cycl + 1
		if not(reg[1] == 0 or flag[2] == 1) then -- A is zero or flag ext-pin 
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x0E+1] = function() 		-- JCN [ITC] 1110
		step_cycl = step_cycl + 1
		if not(flag[1] == 1 or flag[2] == 1) then -- flag carry or flag ext-pin 
			st[1] = (st[1] & 0xF00) | 
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0x0F+1] = function() 		-- JCN [ITCZ] 1111
		step_cycl = step_cycl + 1
		if not(reg[1] == 0 or flag[1] == 1 or flag[2] == 1) then 
			st[1] = (st[1] & 0xF00) |
				rom[2](st[1])	-- fetch new byte into PC
			return
		end
	...
	opset[0xE1+1] = function()		-- WMP (write Acc to main output port)
		step_cycl = step_cycl + 1
		ram[7](reg[1])		-- main output port
	end
	opset[0xE2+1] = function()		-- WRR (write Acc to ROM port)
		step_cycl = step_cycl + 1
		local tbyte = reg[1]
		
		-- bit1 = data (shared between shifter 1 and 2 )
		local data_bit = ((tbyte & 0x02) ~= 0) and 1 or 0
		shiftr1[3](data_bit)
		shiftr2[3](data_bit)
		
		-- bit0 = shifter 1 clock (usually keyboard)
		if (tbyte & 0x01) ~= 0 then
			shiftr1[2]()
		end
		
		-- bit2 = shifter clock (usually printer chip cascade)
		if (tbyte & 0x04) ~= 0 then
			shiftr2[2]()	-- cascaded to prn_shiftr2
		end
	end
	opset[0xE3+1] = function()		-- WPM (write Acc to program output port)
		step_cycl = step_cycl + 1
		ram[9](reg[1])		-- program output port
	end
	...
	opset[0xF8+1] = function()		-- DAC (decrement Acc, no Carry)
		reg[1] = (reg[1] - 1) & 0x0F	-- decrement A
	end
	...
	opset[0xFD+1] = function()		-- DCL (Designate RAM bank)
		ram[10](reg[1] & 0x0F)		-- designate ram chip (0-15)
	end
